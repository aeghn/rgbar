...
use chin_tools::wayland::{into_wl_event, WLCompositor, WLEvent, WLOutput, WLWindow, WLWorkspace};
...
impl Block for WaylandBlock {
    ...
    fn widget(&self, share_info: &WidgetShareInfo) -> Widget {
        ...
        let mut window_container = WindowContainerManager::new().unwrap().init().unwrap();
                    ...
                    Ok(msg) => match msg {
                        OutEvent::WLEvent(event) => match event {
                            WLEvent::WorkspaceFocused(ws) => {
                                if ws.get_output_name().map_or(true, |name| name != output_name) {continue;}
